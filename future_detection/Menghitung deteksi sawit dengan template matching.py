...
sawit = cv2.imread('ramean.jpg')
sawit = cv2.cvtColor(sawit, cv2.COLOR_BGR2RGB)
# panggil dan konversi warna agar sesuai dengan Matplotlib
kebun_sawit = cv2.imread('ramean1.jpg')
kebun_sawit = cv2.cvtColor(kebun_sawit, cv2.COLOR_BGR2RGB) 
plt.subplot(121),plt.imshow(sawit), plt.title('naruto')
plt.subplot(122),plt.imshow(kebun_sawit), plt.title('Sasuke, Naruto dan Sakura')
plt.show()

## membaca gambar utuh untuk dicari
img_rgb = cv2.imread('ramean1.jpg')
img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
## membaca template 
template = cv2.imread('ramean.jpg',0)
