...
einstein = cv2.imread('ramean.jpg')
einstein = cv2.cvtColor(einstein, cv2.COLOR_BGR2RGB) # simpan dengan nama yang 
#sama = ditumpuk

# panggil dan konversi warna agar sesuai dengan Matplotlib
solvay = cv2.imread('ramean1.jpg')
solvay = cv2.cvtColor(solvay, cv2.COLOR_BGR2RGB) 

plt.subplot(121),plt.imshow(einstein), plt.title('Naruto')
plt.subplot(122),plt.imshow(solvay), plt.title('Sasuke, Naruto dan Sakura')
plt.show()
